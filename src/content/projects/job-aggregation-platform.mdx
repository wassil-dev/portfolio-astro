---
title: "Algerian Job Aggregation Platform"
description: "End-to-end data engineering platform aggregating 3,400+ job postings daily from Algerian job boards with intelligent matching and Telegram notifications."
...
A production-ready data engineering platform that automatically collects, processes, and delivers personalized job notifications for the Algerian market. The system scrapes job postings from multiple sources, transforms and stores them in a data warehouse, and matches opportunities with user preferences through an intelligent Telegram bot.
...
**3. Application Layer**
- FastAPI backend for Telegram bot integration
...
### Data Collection
- **Multi-source scraping:** Emploitic (~2,000 jobs) + EmploiPartner (~1,400 jobs)
- **Hourly synchronization** with rate limiting and retry mechanisms
- **Incremental updates** to capture new postings efficiently

### Data Processing
- Automated ETL pipelines with staging → production workflow
...
- Processes 3,400+ jobs in under 5 minutes
...
- **API Framework:** FastAPI, python-telegram-bot
...
- **3,400+ job postings** aggregated and updated hourly
- **Multi-lingual bot** serving users in 3 languages
